...
- `launch_role.py` — Creates one IAM role per product. The role is assumed by `servicecatalog.amazonaws.com` when launching a product. Always includes `AWSCloudFormationFullAccess` (SC launches CFN stacks) plus whatever managed policies the product declares in the TOML. Also supports optional `custom_policy` inline statements to fill gaps in managed policies or to replace them with scoped-down permissions for tighter security. Also includes an inline policy for reading SC template artifacts from S3. Role name follows the convention: `{slug}-{env}-{portfolio}-{product}-lc`.
...
2. Add a `[[portfolio.products]]` entry to the portfolio TOML with `name`, `template`, and `launch_role_policies`. Optionally add `[[portfolio.products.custom_policy]]` entries to fill gaps in managed policies or scope down permissions.
