...
Data generated by on-campus instruments (sequencers, microscopes, sensors) often makes sense to process locally first — avoid uploading raw data to AWS just to process it. Upload the results to S3 for sharing and long-term storage. If your data is already in AWS (public datasets, collaborator-shared buckets), compute in AWS to avoid data transfer costs.
