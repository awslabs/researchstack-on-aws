...
The easiest option is the [S3 console](https://console.aws.amazon.com/s3/) — drag-and-drop files up to 160 GB. For larger uploads (up to 50 TB per object), automation, or scripted pipelines, use the [AWS CLI](https://aws.amazon.com/cli/):
```bash
...
```
The CLI handles multipart uploads automatically for large files — no special setup needed.
...
- **[AWS DataSync](https://aws.amazon.com/datasync/)** — accelerated data transfer between on-prem storage (NFS/SMB), other clouds, and AWS storage services (S3, EFS, FSx). Useful for recurring transfers, migrations, and hybrid workflows — not just one-time migrations.
- **[AWS Data Transfer Terminal](https://aws.amazon.com/data-transfer-terminal/)** — secure physical transfer for petabyte-scale data when network transfer isn't practical.
